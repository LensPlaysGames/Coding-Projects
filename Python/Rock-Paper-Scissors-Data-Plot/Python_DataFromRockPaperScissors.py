...
def get_data():
    ...
    for k in wins:
        ...
        if "Player" in k:
         ...
         # Get # of Times each Play Possibility was Actually, ya know, Played
        if "Play" in k:
...
def print_data(data):
     # Print # of Times each Player has Won
    ...
    print("Player One Won: " + str(data["Player Wins"]["Player One"]) + " times"); print()
     ...
     # Print # of Times each Possibility has been Played
...
def plot_data(data):
    global count
     ...
     # Initialize Data needed to Plot
    ...
    plays_y = [] # Ints Containing # of Times Play has been Played by a Player

     # Convert Data to Format that MatPlotLib understands (list of 'str' or 'int')
    for w in player_wins: # For every dictionary in list, append key to x and value to y list
        wins_x.append(str(w))
        wins_y.append(int(player_wins[w]))

    for p in wins_by_play: # For every dictionary in list, append key to x and value to y list
    ...
    for win_x in wins_x:
        axs[0].text(win_x, player_wins[win_x], str(player_wins[win_x]), ha="center", bbox=dict(fc="red", pad=1, joinstyle="miter"))
    ...
    axs[1].set_title("# of Plays per Play")
    axs[1].set_xlabel("Play")
    axs[1].set_ylabel("# of Times Played")
